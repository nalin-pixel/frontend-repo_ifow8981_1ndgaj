...
export default function Doctors() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setForm({ first_name: '', last_name: '', email: '', phone: '', department: '' })
      await load()
    } catch (e) {
      setError(e.message)
    } finally {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-3">
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td className="py-4" colSpan={5}>No doctors yet</td></tr>
              ) : items.map(d => (
                <tr key={d.id} className="border-t">
                  <td className="py-2 pr-4">{d.first_name} {d.last_name}</td>
      ...
      <div className="bg-white rounded-xl border p-6 shadow-sm">
        <h3 className="font-semibold text-slate-800 mb-4">Add Doctor</h3>
        <form onSubmit={submit} className="space-y-3">
          {['first_name','last_name','department','email','phone'].map((k) => (
            <div key={k}>
